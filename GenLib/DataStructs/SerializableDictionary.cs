	...
	public class SerializableDictionary : Dictionary<string, object>, IXmlSerializable
	{
		...
		public void ReadXml(XmlReader reader)
		{
			...
			while (reader.NodeType != XmlNodeType.EndElement)
			{
				...
				string value = encrypt ? reader.ReadElementString("value").Decrypt(EncryptionKey) 
		...
		public void WriteXml(XmlWriter writer)
		{
			...
			foreach (string key in Keys)
			{
				...
				using (StreamReader sr = new StreamReader(ms))
				{
					...
					writer.WriteElementString("value", encrypt ? sr.ReadToEnd().Encrypt(EncryptionKey) : sr.ReadToEnd());
				}
