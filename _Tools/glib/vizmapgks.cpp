...
TGksColor TVizMapContext::ColorSelPointCross = TGksColor(237, 28, 36);
...
int TVizMapContext::SizePointCross = 3; //15;
...
int TVizMapContext::RndKeyWdN = 6000;
...
void TVizMapContext::PaintPoints(PGks Gks, const int& PointFontSize, 
    ...
    for (int PointN = 0; PointN < Points; PointN++) {
        ...
        if (ZoomRect.IsXYIn(PointX, PointY)) {
            ...
            if (!Point->IsPointNm() || ((Point->GetWgt() < PointWgtThreshold) && !IsSelPointP)) {
                // draw a cross
...
void TVizMapContext::PaintPointCross(PGks Gks, const int& X, 
    ...
    if (IsCatP) {
        // draw a big circle
        const int Size = SizePointCross;
        Gks->SetBrush(TGksBrush::New(SelPointP ? ColorSelPointCross : ColorSelPointCross));
        Gks->FillEllipse(X - Size, Y - Size, X + Size, Y + Size);
        Gks->SetPen(TGksPen::New(TGksColor(0, 0, 0)));
        //Gks->Circle(X, Y, 2*Size);
    } else {
        // draw a small circle
        const int Size = TFlt::Round(SizePointCross / 1.3);
        Gks->SetBrush(TGksBrush::New(SelPointP ? ColorSelPointCross : ColorPointCross));
        Gks->FillEllipse(X - Size, Y - Size, X + Size, Y + Size);
        Gks->SetPen(TGksPen::New(TGksColor(0, 0, 0)));
        //Gks->Circle(X, Y, 2*Size);
    }
...
void TVizMapContext::Export(const TStr& FNm, const TStr& EncoderType, const int& Width, 
    ...
    Paint(BmpGks, ShowPointNmP, PointFontSize, PointNmFontScale, PointWgtThreshold, 
        CatId, ShowCatNmP, ShowKeyWdP, KeyWdFontSize, ShowMgGlassP, 
