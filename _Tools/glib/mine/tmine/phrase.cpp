 ...
 const int& MnOutputNGramFq, TNGramDescV& OutputNGramDescV){
  ...
  for (int TokIdQN=0; TokIdQN<TokIdQVLen; TokIdQN++){
    ...
    if (TokIdQ.Len()==NGramLen){
	  ...
	  TSNGHashDat& NGramItem = StreamNGramHash.AddDat(NGramSig);
	  NGramItem.Val1++;
      int Fq = NGramItem.Val1;

	  // we save in the NGramHash the data vector for the n-gram, but only after the n-gram becomes popular enough
	  if (Fq >= MnOutputNGramFq && NGramLen > 1 && NGramItem.Val2.Len() == 0) {
		TIntV NGramIdV;
		for (int TokIdN=0; TokIdN<NGramLen; TokIdN++)
  		  NGramIdV.Add(TokIdQ[TokIdN]);
		NGramItem.Val2 = NGramIdV;
	  }
...
void TStreamNGramBs::GetFrequentNGrams(TVec<TPair<TUInt, TIntV> > &NGramFqV, int MnNGramFq, int MnNGramLen)
{
	for (int N=0; N < (int) StreamNGramHash.Len(); N++)
	{
		TSNGHashDat NGramItem = StreamNGramHash[N];
		if ((int) NGramItem.Val1 >= MnNGramFq && NGramItem.Val2.Len() >= MnNGramLen)
			NGramFqV.Add(TPair<TUInt, TIntV>(NGramItem.Val1, NGramItem.Val2));
	}
	NGramFqV.Sort(false);
}