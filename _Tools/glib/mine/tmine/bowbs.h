...
public:
  ...
  // postprocessing
  void SortByStr() { WStrToWWgtH.SortByKey(); }
  void SortByWgt() { WStrToWWgtH.SortByDat(false); }
...
public:
  ...
  void UpdateWIdWgt(const int& WId, const double& Wgt){ 
	  IAssert(IsWId(WId)); int WIdN = GetWIdN(WId); WIdWgtKdV[WIdN].Dat = (sdouble) Wgt; }
...
public:
  ...
  const TFltV& GetWordFqV() const { return WordFqV; }
...
public:
  ...
  PBowSpV GetSpVFromHtmlStr(
   const TStr& HtmlStr, const int& Docs, const TFltV& WordFqV) const;
