...
void TVizMapLndMrk::SaveXml(const TStr& XmlFNm) {
    PXmlTok TopTok = TXmlTok::New("vizmap");
    // output vocabualry
    PXmlTok WordsTok = TXmlTok::New("words");
    WordsTok->AddArg("docs", BowDocBs->GetDocs());
    const int Words = BowDocBs->GetWords();
    for (int WId = 0; WId < Words; WId++) {
        TStr WdStr = BowDocBs->GetWordStr(WId);
        const int DocFq = BowDocBs->GetWordFq(WId);
        PXmlTok WordTok = TXmlTok::New("word");
        WordTok->AddArg("str", WdStr);
        WordTok->AddArg("df", DocFq);
        WordsTok->AddSubTok(WordTok);
    }
    TopTok->AddSubTok(WordsTok);
    // output landmarks
    PXmlTok PointsTok = TXmlTok::New("points");
    const int LndMrks = GetLndMrks();
    for (int LndMrkN = 0; LndMrkN < LndMrks; LndMrkN++) {
        PXmlTok PointTok = TXmlTok::New("point");
        // point coordinates
        const TFltV& PointV = GetLndMrkPointV(LndMrkN);
        PointTok->AddArg("x", PointV[0]);
        PointTok->AddArg("y", PointV[1]);
        // sparse vector
        PBowSpV SpV = GetLndMrkSpV(LndMrkN);
        for (int WIdN = 0; WIdN < SpV->GetWIds(); WIdN++) {
            const int WId = SpV->GetWId(WIdN);
            TStr WdStr = BowDocBs->GetWordStr(WId);
            const double Wgt = SpV->GetWgt(WIdN);
            PXmlTok KeyWdTok = TXmlTok::New("keywd");
            KeyWdTok->AddArg("str", WdStr);
            KeyWdTok->AddArg("wgt", Wgt);
            PointTok->AddSubTok(KeyWdTok);
        }
        // document name
        PointTok->AddArg("id", BowDocBs->GetDocNm(SpV->GetDId()));
        PointsTok->AddSubTok(PointTok);
    }
    TopTok->AddSubTok(PointsTok);
    // calculate landscape
    PVizMapLandscape Landscape = TVizMapLandscape::New();
    ...
    // output landscape
    PXmlTok LandscapeTok = TXmlTok::New("landscape");
    const int LandWidth = Landscape->GetXDim();
    const int LandHeight = Landscape->GetYDim();
    LandscapeTok->AddArg("width", LandWidth);
    LandscapeTok->AddArg("height", LandHeight);
    LandscapeTok->AddArg("first_level", Landscape->GetLevel(0));
    LandscapeTok->AddArg("second_level", Landscape->GetLevel(1));
    const TSFltVV& HeightVV = Landscape->HeightVV();
    for (int X = 0; X < LandWidth; X++) {
        for (int Y = 0; Y < LandHeight; Y++) {
            double Val = HeightVV(X, Y);
            PXmlTok CellTok = TXmlTok::New("cell");
            CellTok->AddArg("x", X);
            CellTok->AddArg("y", Y);
            CellTok->AddArg("z", Val);
            LandscapeTok->AddSubTok(CellTok);
        }
    }
    TopTok->AddSubTok(LandscapeTok);
    // make an xml document and save it
    PXmlDoc XmlDoc = TXmlDoc::New(TopTok);
    XmlDoc->SaveTxt(XmlFNm);
}
...
TVizMapXmlDocBs::TVizMapXmlDocBs(const TStr& XmlFNm, const PNotify& Notify) {
    ...
    forever {
        ...
        if (!XmlDoc->IsOk()) { Notify->OnStatus(XmlDoc->GetMsgStr()); break; }
        // display count
...
void TVizMapFactory::MakeFlat(PSVMTrainSet Set, 
    ...
    // prepare matrix B
    TSparseRowMatrix B; B.RowN = EqN; B.ColN = 2*Len; B.RowSpVV.Gen(EqN);
    for (int i = 0; i < B.RowN; i++) B.RowSpVV[i].Gen(4);
    ...
    forever {
        ...
        int k = 0;
        TVec<TIntFltKdV>& RowVV = B.RowSpVV;
        for (int i = 0; i < Len; i++) {
            for (int j = i+1; j < Len; j++) {
                ...
                switch (DistType) {
                    ...
                    case vdtDistMtx: Dij = Set->DotProduct(i, j); break;
...
PVizMap TVizMapFactory::NewVizMap(const PVizMapXmlDocBs& XmlDocBs, 
    ...
    } else if (VizXmlMapType == vxmtDynamicDoc) {
