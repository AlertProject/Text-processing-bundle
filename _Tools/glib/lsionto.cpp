...
void TOntoConcept::GenMedoid(PBowDocWgtBs BowDocWgtBs) {
    ...
    const int TopN = 33;
    TFltIntKdV TopWgtDIdV = WgtDIdV; TopWgtDIdV.Sort(false);
    if (WgtDIdV.Len() > TopN) { WgtDIdV.Trunc(TopN); }
    // store TopN medoids
...
void TOntoAlg::SuggestLwOntoGround(PBowDocBs BowDocBs, PBowDocWgtBs BowDocWgtBs, 
        const TIntV& DIdV, PLwOntoGround LwOntoGround, const int& _NewConcepts, 
    ...
    for (int DIdN = 0; DIdN < DIdV.Len(); DIdN++) {
        ...
        LwOntoGround->ClassifyStr(DocStr, SimTermIdPrV);
    ...
    PLwTermBs TermBs = LwOntoGround->GetLwOnto()->GetTermBs();
...
void TOntology::SuggestSubconcepts(const int& CptId, const PLwOntoGround& LwOntoGround,
        const int& NewConceptN, const bool& UnusedDocsP, TOntoConceptV& NewConceptV) {
    ...
    if (DIdV.Len() > NewConceptN) {
        TOntoAlg::SuggestLwOntoGround(BowDocBs, BowDocWgtBs,
            DIdV, LwOntoGround, NewConceptN, UniqueId, NewConceptV);
    }
}
...
void TOntology::SuggestConceptNm(const int& CptId, const TVec<PLwOntoGround>& LwOntoGroundV, 
    ...
    for (int VocN = 0; VocN < LwOntoGroundV.Len(); VocN++) {
        PLwOntoGround LwOntoGround = LwOntoGroundV[VocN];
        PLwTermBs LwTermBs = LwOntoGround->GetLwOnto()->GetTermBs();
        for (int DIdN = 0; DIdN < DIdV.Len(); DIdN++) {
            const int DId = DIdV[DIdN];
            TStr DocStr = BowDocBs->GetDocStr(DId);
            TSimTermIdPrV SimTermIdPrV; SimTermIdPrV.Sort(false);
    ...
    for (int SuggestNmN = 0; SuggestNmN < SuggestNms; SuggestNmN++) {
        ...
        PLwTermBs LwTermBs = LwOntoGroundV[VocN]->GetLwOnto()->GetTermBs();
        TStr TermNm = LwTermBs->GetTerm(TermId)->GetTermNm();
