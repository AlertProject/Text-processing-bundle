...
public:
	...
	static int GetFldInt(const TStrKdV& FldNmValPrV, const TStr& FldNm, const TStr& DefStr = ""); 
	static uint64 GetFldUInt64(const TStrKdV& FldNmValPrV, const TStr& FldNm, const TStr& DefStr = ""); 
...
ClassTE(TSASFunFPath, TSAppSrvFun)//{
private:
	TStr BaseUrl;
	TStr FPath;
...
private:	
	TStr ContType;	
public:
	TSASFunImg(const TStr& _ContType): TSAppSrvFun("seq-jpg", saotCustom), ContType(_ContType) { }
	static PSAppSrvFun New(const TStr& ContType) { return new TSASFunImg(ContType); }
