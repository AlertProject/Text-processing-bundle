...
int TWebPgFetchEvent::TimeOut = 30;
...
TWebPgFetchEvent::TWebPgFetchEvent(
 TWebPgFetch* _Fetch, const int& _FId, const PUrl& _Url, 
 const TStr& _ProxyStr, const TStr& _UserAgentStr):
  TSockEvent(),
...
void TWebPgFetchEvent::OnRead(const int&, const PSIn& SIn){
  Sock->PutTimeOut(TWebPgFetchEvent::TimeOut*1000);
  SockMem+=SIn;
...
void TWebPgFetchEvent::OnWrite(const int&) {
  if (Sock->IsUnsent()){
    Sock->Send(NULL);
  }
}
...
void TWebPgFetch::OpenConn(const int& FId, const PUrl& Url){
  PSockEvent Event=PSockEvent(new TWebPgFetchEvent(this, FId, Url, ProxyStr, UserAgentStr));
...
/*void TWebPgToFileFetch::OnFetch(const int&, const PWebPg& WebPg){
  printf("Fetched: %s\n", WebPg->GetUrlStr().CStr());
  // save web-page as http
  if (OutHttpP){WebPg->SaveAsHttp(TFile::GetUniqueFNm(TStr::PutFExt(OutFNm, ".http")));}
  // save http-body
  if (OutHttpBodyP){WebPg->SaveAsHttpBody(TFile::GetUniqueFNm(TStr::PutFExt(OutFNm, ".body")));}
  // save web-page as xml
  if (OutXmlP){
    TStr HtmlStr=WebPg->GetHttpBodyAsStr();
    THtmlDoc::SaveHtmlToXml(HtmlStr, TFile::GetUniqueFNm(TStr::PutFExt(OutFNm, ".xml")), WebPg->GetUrlStr(),
     OutXmlTextP, OutXmlUrlP, OutXmlToksP, OutXmlTagsP, OutXmlArgsP);
  }
  // save web-page as text
  if (OutTxtP){
    TStr HtmlStr=WebPg->GetHttpBodyAsStr();
    THtmlDoc::SaveHtmlToTxt(HtmlStr, TFile::GetUniqueFNm(TStr::PutFExt(OutFNm, ".txt")), WebPg->GetUrlStr(),
     OutTxtUrlP, OutXmlTagsP);
  }
  // output to screen
  if (OutScrP){
...
void TWebPgToFileFetch::OnError(const int&, const TStr& MsgStr){
  // save error-message as http-body
  if (OutHttpP){
    TFOut SOut(TFile::GetUniqueFNm(TStr::PutFExt(OutFNm, ".http"))); TFileId SOutFId=SOut.GetFileId();
    fprintf(SOutFId, "Error: %s\n", MsgStr.CStr());
  }
  // save error-message as http
  if (OutHttpBodyP){
    TFOut SOut(TFile::GetUniqueFNm(TStr::PutFExt(OutFNm, ".body"))); TFileId SOutFId=SOut.GetFileId();
    fprintf(SOutFId, "Error: %s\n", MsgStr.CStr());
  }
  // save error-message as xml
  if (OutXmlP){
    // create output file
    TFOut SOut(TFile::GetUniqueFNm(TStr::PutFExt(OutFNm, ".xml"))); TFileId SOutFId=SOut.GetFileId();
    // open top tag
    fprintf(SOutFId, "<WebPage>\n");
    // save error message
    TStr XmlMsgStr=TXmlLx::GetXmlStrFromPlainStr(MsgStr);
